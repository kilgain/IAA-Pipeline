...

{
  ...
  metaboliteMatcher = function(massesToAnalyze, sorghumMetabolites, listOfVecOfStarts, listOfVecOfStops) {
    pdf(file = paste("/home/lconnelly/outputPlots/barplotForMass", myMass, ".pdf", sep = ""),
        width = 8,
        height = 5)
...
print(args)
print( " is args")
vecOfMasses = unlist(strsplit(args, ","))
#code in path to BiggestTable
BiggestTable = read.csv("/home/lconnelly/dataTable/36FileTable.csv", stringsAsFactors = T)

for (i in 1:length(vecOfMasses)) {
  myMass = vecOfMasses[i]
  # myMass = 182.1230
  print(myMass)
  ppmAllowed = .000010
  rangeAllowed = ppmAllowed * myMass
  mySubset = BiggestTable %>% filter(mass < (myMass + rangeAllowed) &
                                       mass > (myMass - rangeAllowed))
  ...
  if (length(vecOfStartsFinal > 0)) {
    pdf(file = paste("/home/lconnelly/outputPlots/", myMass, "Bounds.pdf", sep = ""),
        width = 8,
        height = 5)
    plot(
      mySubsetOG$scan,
    ...
    for (z in 1:length(vecOfStartsFinal)) {
  ...
  if (length(vecOfStops) == 0) {
    vecOfStops = 3000
  }
  #implment for loop here to account for multiple vecOfStarts/Stops
  numPointsAtEachShift <-
    metaboliteMatcher(myMass + .0012,
                      BiggestTable,
                      vecOfStartsFinal[1],
                      vecOfStopsFinal[1])
  
  #length(numPointsAtEachShift[[1]])
  
  #Make this a function: arguments... numPointsEachShift, myMass,
  vecOfMaxRatios = vector()
  for (p in 1:length(numPointsAtEachShift[[1]])) {
    rangeAllowed = .000005 * myMass
    massOfSorghumMetabolite = myMass - (p) * (1.00336)
    sorghumRegionOfInterest <-
      BiggestTable %>% filter(
        mass < (massOfSorghumMetabolite + rangeAllowed) &
          mass > (massOfSorghumMetabolite - rangeAllowed)
      )
    sorghumRegionOfInterest <-
      sorghumRegionOfInterest %>% filter(scan < (vecOfStops[1]) &
                                           scan > (vecOfStarts[1]))
    massOfSorghumMetabolite <- mean(sorghumRegionOfInterest$mass)
    
    if (length(sorghumRegionOfInterest$mass > 200)) {
      M0 <-
        BiggestTable[BiggestTable$mass > ((massOfSorghumMetabolite) - ((massOfSorghumMetabolite) *
                                                                         (.000005))) &
                       BiggestTable$mass < ((massOfSorghumMetabolite) + ((massOfSorghumMetabolite) *
                                                                           (.000005))), ]
      ...
      M1 <-
        BiggestTable[BiggestTable$mass > ((massOfSorghumMetabolite + 1.00336) -
                                            ((massOfSorghumMetabolite + 1.00336) * (.000005))) &
                       BiggestTable$mass < ((massOfSorghumMetabolite + 1.00336) + ((massOfSorghumMetabolite +
                                                                                      1.00336) * (.000005))), ]
      ...
      #M0 + M1 plots
      if(((maxSignalM1 / maxSignalM0) / 1.109) < 1) {
        pdf(file = paste("/home/lconnelly/outputPlots/", massOfSorghumMetabolite, "M0M1.pdf", sep = ""),
            width = 8,
            height = 5)
        {
          plot(
            ...
            xlim = c(-10, 3100),
            ylab = "Intensity",
            main = paste("Retention Time for Mass", massOfSorghumMetabolite, " and shift = ", p)
          )
          plot(
            M0$mass,
            M0$intensity,
            ...
            # xlim = c(-10, 3100),
            ylab = "Intensity",
            main = paste("Mass", massOfSorghumMetabolite, " for shift", p)
          )
          ...
          plot(
            ...
            # xlim = c(-10, 3100),
            ylab = "Intensity",
            main = paste("Mass", massOfSorghumMetabolite, "+1.00336 for shift", p)
          )
        }
        dev.off()
      }
      vecOfMaxRatios <- c(vecOfMaxRatios, ((maxSignalM1 / maxSignalM0) * 100 / 1.109))
      
      
      print(
        paste(
          "number of carbons in this",
          massOfSorghumMetabolite,
          " dalton compound is equal to:",
          ((maxSignalM1 / maxSignalM0) * 100 / 1.109),
          " For p=",
          p
        )
      )
      ...
      if (length(M0$intensity) > 100 & length(M1$intensity) > 100 &  ((maxSignalM1 / maxSignalM0) / 1.109) < 1) {
        vecOfRatios = vector()
        for (j in 1:10000)
        {
          # print(j)
          ...
          M1vM0ratio = max(subsetM1$intensity) / max(subsetM0$intensity / 1.109)
          vecOfRatios = c(vecOfRatios,  M1vM0ratio)
        }
        vecOfRatios <- sort(vecOfRatios, decreasing = F)
        vecOfRatios <- vecOfRatios[vecOfRatios < 1]
        if(length(vecOfRatios) > 100) {
        ...
        pdf(file = paste("/home/lconnelly/outputPlots/histForMass", massOfSorghumMetabolite, "atShift",p, ".pdf", sep = ""),
            width = 8,
            height = 5)
        hist(
          vecOfRatios,
          breaks = seq(from=0, to=1, by=.005),
          xlim = c(0,.5),
          main = paste(
            "VectorOfRatios For Mass=",
            massOfSorghumMetabolite,
            " carbon shift of ",
            p,
            " from ",
        ...
        print(paste((valueToPlot-.01)*100, " is estimated number of carbons obtained by subsetting for p =", p))
        abline(v = p * .0109, col = "green")
        abline(v = valueToPlot-.01, col = "red")
        dev.off()
        }
      }
    }
  }
  smallestDifference = c(0, 10)
  for(v in 1:length(vecOfMaxRatios)) {
    if(vecOfMaxRatios[v]-v < smallestDifference[2]) {
      smallestDifference[2] = vecOfMaxRatios[v]
      smallestDifference[1] = v
    }
  }
  print(paste("predicted number of carbons for Mass", myMass, " is ", smallestDifference[1]))
}
