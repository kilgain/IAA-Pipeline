...
#	files = [yaml['samplesDirectory'] + '/' + runNumber + '/' + f for f in files]
	...
	#if x == False:
		for f in files:
			processed.append(client.submit(runPythonScript, root + "/step1Conversion/create_the_hdf5_files.py", f, outputPath))
		progress(processed)
		client.gather(processed)
		client.shutdown()
	...
	#test to see if new column has already been added
	#test = [i for i in files if ".hdf5newColumn" in i]
	#test = test[0]
	...
	#config = /home/lconnelly/Metabolomics/outputs/hdf5FilesNewColumn/Analysis1
		...
		for f in files:
			processed.append(client.submit(runPythonScript, root + "/step2Isolock/python_script_determine_offset.py", f, refFile))
		progress(processed)
		client.gather(processed)
		client.shutdown()
...
#		path = outputPathPositive
#		x = os.path.isdir(path)
#		if x == False:
#			os.mkdir(path)
			processed.append(client.submit(runPythonScript, root + "/step3Autocredential/autocredential.py", "positive", path, outputPathPositive, outputPathNegative))
...
#		if x == False:
#			os.mkdir(config)
			processed.append(client.submit(runPythonScript, root + "/step3Autocredential/autocredential.py", "negative", path, outputPathPositive, outputPathNegative))
...
#		os.mkdir(config)
		...
		print(root + "/step3Autocredential/centroidFxn.R")

		for f in files:
			print(f)
			print(outputPath)
			#processed.append(client.submit(runScript, root + "/step3Autocredential/centroidFxn.R", f))
			processed.append(client.submit(runScript, root + "/step3Autocredential/centroidFxn.R", f, outputPath))
...
#	if not os.path.isdir(config['tempFiles']):
		for f in files:
			processed.append(client.submit(runPythonScript, root + "/step4Subset/create_all_the_temp_files.py", f, maxMass, outputPath))
...
#	massDirectories = massDirectories[0:200]
	...
	if toRun[9] == 1:
		for m in massDirectories:
			processed.append(client.submit(runScript, root + "/step5Merge/read_in_the_masses_tempfiles.R", m, yaml['fileList'], plotPath, vecOfIntensitiesPath))
